...
async function sendCancelTokenEmail(email: string, token: string): Promise<boolean> {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;color:#18181b">
      <h2 style="color:#db2777">Confirm subscription cancellation</h2>
      <p>We received a request to cancel your <strong>SafeUnfollow Premium</strong>
         subscription.</p>
      ...
      <p style="font-size:13px;color:#71717a">
        This code expires in 15 minutes. If you did not request cancellation,
        you can safely ignore this email — your subscription will not be cancelled.
      </p>
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: EMAIL_FROM,
      to: email,
      subject: 'Your SafeUnfollow cancellation confirmation code',
      html,
    }),
  });
