...
function UserList({ users, sort }: { users: InstagramUser[]; sort: SortKey }) {
  ...
  if (sorted.length === 0) {
    return <p className="text-center text-slate-400 dark:text-slate-500 py-10 text-sm">No accounts found.</p>
  }
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="text-center">
            ...
            <h2 className="text-lg font-semibold text-[#1E293B] dark:text-white mb-2">No data yet</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-6">Upload your Instagram data file to get started.</p>
            <Link
              href="/upload"
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 max-w-3xl mx-auto px-4 py-8 w-full">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-bold text-[#1E293B] dark:text-white">Your Analysis</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {[
            { icon: Users, label: 'Followers', value: followers.length, color: 'text-blue-500' },
            { icon: UserPlus, label: 'Following', value: following.length, color: 'text-purple-500' },
            { icon: ArrowLeftRight, label: 'Mutual', value: mutual.length, color: 'text-green-500' },
            { icon: UserMinus, label: "Don't Follow Back", value: notFollowingBack.length, color: 'text-red-500' },
        ...
        <div className="flex gap-1 mb-4 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl">
          {[
            { key: 'notFollowingBack', label: "Don't Follow Back", count: notFollowingBack.length },
            { key: 'iDontFollowBack', label: "I Don't Follow Back", count: iDontFollowBack.length },
            { key: 'changes', label: 'Changes', premium: true },
          ].map((t) => (
            <button
              key={t.key}
              onClick={() => {
                if (t.premium) { setShowPremium(true); return }
                setTab(t.key as Tab); setSearch('')
              }}
            ...
            >
              {t.premium && <Lock size={11} />}
              {t.label}
              {t.count !== undefined && (
                <span className="bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 text-xs px-1.5 py-0.5 rounded-full">{t.count}</span>
              )}
            </button>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700 shadow-sm px-4">
          <UserList users={filtered} sort={sort} />
        </div>
        ...
        <div className="mt-6 bg-gradient-to-r from-[#1A73E8]/10 to-purple-500/10 border border-[#1A73E8]/20 rounded-xl p-4 flex items-center justify-between">
          <div>
            <p className="font-semibold text-[#1E293B] dark:text-white text-sm">Unlock Premium Features</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Unlimited snapshots, CSV export, change history</p>
          </div>
          <button
